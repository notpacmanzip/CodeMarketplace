...
def edit_profile():
    ...
    return render_template('profile.html', user=current_user, form=form, editing=True)
...
def product_list():
    ...
    if language:
        products_query = products_query.filter(Product.programming_languages.ilike(f'%{language}%'))
    ...
    return render_template('products/list.html', products=products, form=form, 
                         categories=categories, current_filters={
                             'query': query, 'category': category_id, 'language': language,
...
def product_detail(product_id):
    ...
    has_purchased = False
    if current_user.is_authenticated:
        has_purchased = Purchase.query.filter_by(buyer_id=current_user.id, product_id=product_id).first() is not None
    ...
    return render_template('products/detail.html', product=product, reviews=reviews, 
                         has_purchased=has_purchased, has_reviewed=has_reviewed)
...
def create_product():
    ...
    if form.validate_on_submit():
        ...
        flash('Product created successfully! It will be reviewed before going live.', 'success')
...
def add_review(product_id):
    ...
    # Check if user has purchased this product
    purchase = Purchase.query.filter_by(buyer_id=current_user.id, product_id=product_id).first()
    if not purchase:
...
def checkout(product_id):
    ...
    existing_purchase = Purchase.query.filter_by(buyer_id=current_user.id, product_id=product_id).first()
...
def seller_dashboard():
    ...
    products = Product.query.filter_by(seller_id=current_user.id).order_by(desc(Product.created_at)).all()
    
    # Get sales statistics
    sales = Purchase.query.join(Product).filter(Product.seller_id == current_user.id).all()
    total_earnings = sum([sale.amount_paid for sale in sales])
    total_sales = len(sales)
    
    # Get recent reviews
    recent_reviews = Review.query.filter_by(seller_id=current_user.id).order_by(desc(Review.created_at)).limit(5).all()
    
    return render_template('dashboard/seller.html', products=products, total_earnings=total_earnings,
                         total_sales=total_sales, recent_reviews=recent_reviews)
...
def admin_dashboard():
    ...
    pending_products = Product.query.filter_by(status='pending').order_by(desc(Product.created_at)).all()
    ...
    return render_template('dashboard/admin.html', pending_products=pending_products,
...
def inbox():
    ...
    for conv in conversations:
        conv_id = conv[0]
        last_message = Message.query.filter_by(conversation_id=conv_id).order_by(desc(Message.created_at)).first()
    ...
    conversation_data.sort(key=lambda x: x['last_message'].created_at, reverse=True)
...
def conversation(conversation_id):
    ...
    return render_template('messages/conversation.html', messages=messages, 
                         conversation_id=conversation_id, other_user=other_user)
...
def send_message():
    ...
    if form.validate_on_submit():
        ...
        if not conversation_id:
            conversation_id = generate_conversation_id(current_user.id, form.recipient_id.data)
...
def contact_user(user_id):
    ...
    return render_template('messages/conversation.html', form=form, other_user=user, 
...
def create_default_data():
    ...
    if Category.query.count() == 0:
        categories = [
            Category(name='Web Development', description='Web applications, websites, and web tools'),
            Category(name='Mobile Apps', description='iOS, Android, and cross-platform mobile applications'),
            Category(name='Desktop Software', description='Desktop applications and utilities'),
            Category(name='APIs & Libraries', description='APIs, libraries, and code packages'),
            Category(name='Games', description='Video games and game development tools'),
            Category(name='DevOps & Tools', description='Development tools, CI/CD, and automation'),
            Category(name='Data Science', description='Data analysis, machine learning, and analytics tools'),
            Category(name='Other', description='Other software products and tools')
        ]
